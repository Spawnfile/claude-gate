...
// src/hook/session-start.ts
async function runSessionStart(stdinJson, opts) {
  let p;
  try {
    p = JSON.parse(stdinJson);
  } catch {
    return null;
  }
  ...
  if (socketOverride !== void 0) {
    resolvedSocket = socketOverride;
  } else if (activeSessionOverride !== void 0) {
    resolvedSocket = activeSessionOverride?.socket_path ?? socketPathFor(p.session_id);
  } else {
  ...
  try {
    const r = await rpc(resolvedSocket, {
      ...
      payload: {
        ...
        cwd: p.cwd,
        ...p.transcript_path !== void 0 ? { transcript_path: p.transcript_path } : {},
        ...p.permission_mode !== void 0 ? { permission_mode: p.permission_mode } : {}
      }
    });
    const resp = r;
...
async function rpc(socketPath, msg) {
