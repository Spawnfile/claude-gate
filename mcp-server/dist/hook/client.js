import{createRequire as __cgCreateRequire}from'node:module';const require=__cgCreateRequire(import.meta.url);
...
var active_session_exports = {};
__export(active_session_exports, {
  activeSessionPath: () => activeSessionPath,
  clearActiveSession: () => clearActiveSession,
  readActiveSession: () => readActiveSession,
  writeActiveSession: () => writeActiveSession
});
import { existsSync as existsSync2, mkdirSync as mkdirSync2, readFileSync as readFileSync2, unlinkSync, writeFileSync } from "node:fs";
import { homedir as homedir2 } from "node:os";
import { join as join2, dirname as dirname2 } from "node:path";
function activeSessionPath(homeOverride) {
  const base = homeOverride ?? homedir2();
  return join2(base, ".claude", "gate", "active-session.json");
}
function writeActiveSession(session, homeOverride) {
...
function readActiveSession(homeOverride) {
  const path = activeSessionPath(homeOverride);
...
function clearActiveSession(homeOverride) {
  const path = activeSessionPath(homeOverride);
...
var ActiveSessionSchema;
var init_active_session = __esm({
  "src/active-session.ts"() {
    "use strict";
    init_zod();
    init_log();
    ActiveSessionSchema = external_exports.object({
      socket_path: external_exports.string().min(1),
      session_id: external_exports.string().min(1),
      project_root: external_exports.string().min(1),
      started_at_ms: external_exports.number().int().nonnegative()
    });
  }
});
...
var MAX_FRAME_BYTES = 1 * 1024 * 1024;
...
var FrameDecoder = class extends EventEmitter {
...
// src/policy/tool-call.ts
init_zod();
var ToolCallEnvelopeSchema = external_exports.object({
  protocol_version: external_exports.literal("1"),
  tool_name: external_exports.string().min(1),
  tool_use_id: external_exports.string().min(1),
  parameters: external_exports.unknown(),
  session_id: external_exports.string().min(1),
  cwd: external_exports.string().min(1),
  timestamp_ms: external_exports.number().int().nonnegative(),
  mcp_server_id: external_exports.string().min(1).nullable(),
  permission_mode: external_exports.string().optional()
});
var MCP_TOOL_RE = /^mcp__plugin_(?<plugin>[^_]+(?:-[^_]+)*)_(?<server>[^_]+(?:-[^_]+)*)__(?<tool>.+)$/;
function deriveMcpServerId(toolName) {
  const m = MCP_TOOL_RE.exec(toolName);
  if (!m || !m.groups) return null;
  return m.groups["server"] ?? null;
}

// src/hook/protocol.ts
import { join } from "node:path";
import { tmpdir, homedir } from "node:os";
var IPC_TIMEOUT_MS = 500;
...
function lookupEnv(cache, tool, mcpServerId) {
  if (cache === null) return "unknown";
  for (const r of cache.resources) {
    for (const m of r.matchers) {
      const hasConstraint = m.tool !== void 0 || m.mcp_server_id !== void 0;
      if (!hasConstraint) continue;
      const mcpOk = m.mcp_server_id === void 0 || mcpServerId !== null && m.mcp_server_id === mcpServerId;
      const toolOk = m.tool === void 0 || globMatch(m.tool, tool);
      if (mcpOk && toolOk) {
        return r.env;
      }
    }
...
function fallbackDecision(env, override) {
...
// src/hook/fallback-log.ts
import { mkdirSync, appendFileSync } from "node:fs";
import { dirname } from "node:path";
function appendFallback(path, entry) {
  try {
...
async function runHook(opts) {
  let parsed;
  try {
    parsed = JSON.parse(opts.stdinJson);
  } catch {
  ...
  let resolvedSocket;
  if (opts.socketOverride !== void 0) {
    resolvedSocket = opts.socketOverride;
  } else if (opts.activeSessionOverride !== void 0) {
    resolvedSocket = opts.activeSessionOverride?.socket_path ?? socketPathFor(parsed.session_id);
  } else {
    const { readActiveSession: readActiveSession2 } = await Promise.resolve().then(() => (init_active_session(), active_session_exports));
    const active = readActiveSession2();
  ...
  try {
    const response = await sendDecide(resolvedSocket, {
      ...
      payload: {
        ...
        ...parsed.permission_mode !== void 0 ? { permission_mode: parsed.permission_mode } : {}
      }
    });
...
function runFallback(parsed, reason, opts) {
  ...
  if (decision === "DENY") {
    return {
      exitCode: EXIT_DENY,
      stderr: `[claude-gate] gate-mcp unreachable; failing closed for ${env} (${parsed.tool_name})
`
    };
  }
  return { exitCode: EXIT_ALLOW };
}
async function sendDecide(socketPath, request) {
...
function mapDecisionToOutcome(response) {
    ...
    return {
      ...
      stdout: JSON.stringify({
        hookSpecificOutput: {
          ...
          permissionDecisionReason: rationale || "claude-gate flagged this tool call for confirmation."
        }
      })
    };
  }
